...
public class Movement_Agent : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        if (child.isAlive)
        {
            ...
            {
                child.animator.SetFloat("RunVertical", Mathf.Abs(rb.linearVelocity.normalized.y));
                child.animator.SetFloat("RunHorizontal", Mathf.Abs(rb.linearVelocity.normalized.x));
            }
        }
    }
    ...
    private IEnumerator Push(Vector2 normalized, float seconds)
    {
        ...
        rb.linearVelocity  = normalized * child.stats.speed * Time.timeScale * 0.5f;
